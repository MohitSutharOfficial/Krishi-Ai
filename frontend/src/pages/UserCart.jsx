...
function UserCart() {
    ...
    return (
        <div className='bg-gradient-to-b from-[#FEFAE0] to-white min-h-screen pt-20'>
            <div className='max-w-6xl mx-auto p-4 py-8'>
                <div className='flex flex-col md:flex-row justify-between items-center mb-8'>
                    <div>
                        <h1 className="text-3xl md:text-4xl font-bold text-gray-100 mb-2">{t('your_cart')}</h1>
                    ...
                    {cart.length > 0 && (
                        <button
                            className='mt-4 md:mt-0 bg-red-500 text-gray-100 px-6 py-3 rounded-lg hover:bg-red-600 transition duration-300 flex items-center shadow-md'
                            onClick={clearCart}
                        >
                    ...
                    <div>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                            {cart.map((product, index) => (
                                <div key={index} className="bg-green-50 rounded-xl shadow-sm overflow-hidden hover:shadow-md transition-shadow duration-300 flex flex-col h-full">
                                    <div className="relative h-48 overflow-hidden">
                                        ...
                                        <div className="absolute top-0 right-0 m-2">
                                            <button
                                                ...
                                                className="bg-green-50 hover:bg-gray-800 text-red-500 p-2 rounded-full shadow-sm transition-colors"
                                    ...
                                    <div className="p-5 flex-grow">
                                        ...
                                        <div className="text-sm text-gray-500 mb-2">
                                            <span className="inline-block bg-gray-600 rounded-full px-3 py-1 text-xs font-semibold text-gray-200 mr-2">
                                                {product.category}
                                            </span>
                                        </div>
                                    </div>

                                    <div className="px-5 py-3 bg-green-50 border-t border-gray-500 flex justify-between">
                                        <button
                                            onClick={() => window.location.href = `/product/${product._id}`}
                                            className="text-blue-400 hover:text-blue-500 text-sm font-medium flex items-center"
                                        >
                                        ...
                                        <button
                                            className="text-green-400 hover:text-green-500 text-sm font-medium flex items-center"
                        ...
                        <div className="bg-green-50 rounded-xl shadow-sm p-6 mb-8">
                            <h2 className="text-xl font-bold mb-4 text-gray-100">{t('order_summary')}</h2>
                            <div className="border-t border-gray-500 pt-4">
                                ...
                                <div className="border-t border-gray-500 mt-4 pt-4 flex justify-between">
                                    <span className="text-lg font-bold text-gray-100">{t('total')}:</span>
                                    <span className="text-lg font-bold text-green-400">₹{cart.reduce((total, item) => {
                                        const price = item.price ? parseFloat(item.price.replace(/[^\d.]/g, '')) : 0;
                                        return total + price;
                                    }, 0).toFixed(2)}</span>
                                </div>
                            </div>
                            <button className="w-full mt-6 bg-green-500 text-gray-800 py-3 rounded-lg hover:bg-green-700 transition-colors shadow-sm font-medium">
